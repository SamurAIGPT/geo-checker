...
import { FaCoins, FaUser, FaSignOutAlt, FaGoogle, FaBars, FaTimes, FaGlobe } from "react-icons/fa";
import { SiVercel } from "react-icons/si";
import clsx from "clsx";

const navLinks = [
  { name: "GEO Studio", href: "/" },
...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 flex items-center justify-between px-4 sm:px-6 py-3.5 bg-zinc-950 border-b border-zinc-800 backdrop-blur-md text-zinc-100 flex-shrink-0">
      {/* Brand logo */}
      <div className="flex items-center gap-5 sm:gap-7 min-w-0">
        <Link href="/" className="flex items-center gap-2 font-extrabold tracking-tight text-white flex-shrink-0 group">
          <div className="h-8 w-8 rounded-lg bg-gradient-to-tr from-violet-600 to-fuchsia-50 flex items-center justify-center text-white shadow-lg shadow-violet-500/20 group-hover:scale-105 transition-transform duration-200">
            <FaGlobe className="text-sm animate-pulse" />
          </div>
          <span className="text-sm sm:text-base leading-none">
            Geo<span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-fuchsia-400 font-black">Checker</span>
          </span>
        </Link>
      ...
      <div className="hidden md:flex items-center gap-2 sm:gap-3 flex-shrink-0">
        {/* Deploy button */}
        <a
          href="https://vercel.com/new/clone?repository-url=https://github.com/SamurAIGPT/geo-checker"
          target="_blank"
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 z-[200] flex flex-col p-4 bg-zinc-950/95 border-b border-zinc-800 backdrop-blur-lg md:hidden gap-3 shadow-xl">
            ...
            {session?.user ? (
              <div className="flex items-center justify-between px-3 py-1">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold text-zinc-400 hover:text-red-400 hover:bg-red-955/20 transition-all"
                >
                  <FaSignOutAlt className="text-xs" />
                  <span>Sign out</span>
                </button>
